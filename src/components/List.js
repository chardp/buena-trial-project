...
const List = ({list, title, onBreedSelect, breedExtraImages, favoriteImages, handleFavorite, onVote }) => {
	const classes = useStyles();	

	const renderedList = list.map((listItem, index) => {
		return <ListItem key={listItem.breeds[0].id}  onVote={onVote} favoriteImages={favoriteImages} handleFavorite={handleFavorite} onBreedSelect={onBreedSelect} breed={listItem.breeds[0]} imageSrc={listItem.url} index={index} breedExtraImages={breedExtraImages} />
	});
